...
export default function Post({ post }: { post: PostType }) {
  ...
  //פונקציה המעדכנת את כמות הלייקים, עדיין לא שולח נתונים לדאטה בייס
  function onCLickLike(
    event: React.MouseEvent<HTMLDivElement, MouseEvent>,
    like: boolean
  ) {
    event.stopPropagation();
    //בודק איזה אייקון נלחץ
    if (like) {
      // בודק שהאייקון השני לא נלחץ
      if (isOneDisLike) {
        //בודק האם האייקון הנוכחי נלחץ
        if (isOneLike) {
  ...
  return (
    <div className="post" onClick={() => navigate(`/post/${post.id}`)}>
      <div className="head-post">
        <img className="post-img" src={post.imgUrl} alt="logo-img" />
      ...
      <div className="likes">
        <div className="like" onClick={(e) => onCLickLike(e, true)}>
          <img className="like-img" src={likeImg} alt="likeImg" />
          <div className="counterLike">{countLike}</div>
        </div>
        <div className="like" onClick={(e) => onCLickLike(e, false)}>
